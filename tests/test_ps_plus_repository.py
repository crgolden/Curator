...
def test_the_window_opens_at_the_earliest_completion_when_every_category_reports_one():
    earliest, latest = _completions(2)

    assert _earliest_only_when_no_value_is_missing([latest, earliest]) == earliest


def test_one_category_without_a_previous_completion_withholds_the_window_from_all_of_them():
    (only_completion,) = _completions(1)

    assert _earliest_only_when_no_value_is_missing([only_completion, None]) is None


def test_no_categories_at_all_withholds_the_window():
