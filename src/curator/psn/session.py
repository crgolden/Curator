...
AUTHORIZE_URL: Final = f"{_AUTH_BASE}/authorize"
TOKEN_URL: Final = f"{_AUTH_BASE}/token"
...
REDIRECT_URI: Final = "com.scee.psxandroid.scecompcall://redirect"

ACCESS_TYPE_PARAM: Final = "access_type"
OFFLINE_ACCESS_TYPE: Final = "offline"
"""What makes PSN issue a refresh token at all; without it every link is an access-token-only session."""
CODE_PARAM: Final = "code"
ERROR_PARAM: Final = "error"
GRANT_TYPE_PARAM: Final = "grant_type"
AUTHORIZATION_CODE_GRANT: Final = "authorization_code"
REFRESH_TOKEN_GRANT: Final = "refresh_token"
NPSSO_COOKIE_NAME: Final = "npsso"
...
RETRYABLE_METHODS = frozenset({GET_METHOD})
...
def npsso_cookie(npsso: str | None) -> str:
    return f"{NPSSO_COOKIE_NAME}={npsso}"
...
class PsnSession:
    ...
    async def _reauthenticate(self) -> bool:
        ...
        """
        if self.token_response is not None and self.token_response.get(REFRESH_TOKEN_KEY):
            try:
                await self._refresh()
                return True
            except PsnAuthError:
    ...
    async def _ensure_fresh(self) -> None:
        ...
        if self.token_response is None:
            ...
            return
        if self.token_response.get(ACCESS_TOKEN_EXPIRES_AT_KEY, 0) > time.time():
            return
        await self._refresh()
    ...
    async def _bootstrap(self) -> None:
        ...
        await self._exchange(grant_type=AUTHORIZATION_CODE_GRANT, code=code)
    ...
    async def _authorization_code(self) -> str:
        ...
        headers = {
            COOKIE_HEADER: npsso_cookie(self._npsso),
            "X-Requested-With": "com.scee.psxandroid",
        }
        params = {
            ACCESS_TYPE_PARAM: OFFLINE_ACCESS_TYPE,
            "client_id": _CLIENT_ID,
            "redirect_uri": REDIRECT_URI,
            SCOPE_KEY: _SCOPE,
            "response_type": CODE_PARAM,
            "cid": self._cid,
        }
    ...
    async def _refresh(self) -> None:
        ...
        """
        if self.token_response is None or not self.token_response.get(REFRESH_TOKEN_KEY):
            raise PsnAuthError("No refresh token available.")
        await self._exchange(grant_type=REFRESH_TOKEN_GRANT, refresh_token=self.token_response[REFRESH_TOKEN_KEY])
    ...
    async def _exchange(self, *, grant_type: str, code: str | None = None, refresh_token: str | None = None) -> None:
        ...
        headers = {
            AUTHORIZATION_HEADER: _BASIC_AUTH,
            CONTENT_TYPE_HEADER: FORM_URLENCODED_CONTENT_TYPE,
            USER_AGENT_HEADER: _TOKEN_USER_AGENT,
        }
        data = {GRANT_TYPE_PARAM: grant_type, SCOPE_KEY: _SCOPE, "token_format": "jwt"}
        if code is not None:
            data.update({CODE_PARAM: code, "redirect_uri": REDIRECT_URI, "cid": self._cid})
        if refresh_token is not None:
            data[REFRESH_TOKEN_KEY] = refresh_token

        response = await self._throttled_request(POST_METHOD, TOKEN_URL, headers=headers, data=data)
        ...
        expires_in = token.get(EXPIRES_IN_KEY)
        if not isinstance(expires_in, (int, float)):
            raise PsnAuthError("PSN's token response carried no usable expires_in.")
        now = time.time()
        token[ACCESS_TOKEN_EXPIRES_AT_KEY] = expires_in + now
        if REFRESH_TOKEN_EXPIRES_IN_KEY in token:
            token[REFRESH_TOKEN_EXPIRES_AT_KEY] = token[REFRESH_TOKEN_EXPIRES_IN_KEY] + now
    ...
    async def get(
        ...
        *,
        ...
        """
        return await self._request(GET_METHOD, url, params=params, headers=headers)
    ...
    async def post(
        ...
        *,
        ...
        :raises PsnAuthError: If the response is ``401``/``403`` (an expired/invalid token or authorization).
        """
        return await self._request(POST_METHOD, url, params=params, headers=headers, json=json, data=data)
    ...
    async def patch(
        ...
        *,
        ...
        :raises PsnAuthError: If the response is ``401``/``403`` (an expired/invalid token or authorization).
        """
        return await self._request(PATCH_METHOD, url, headers=headers, json=json)
    ...
    async def put(self, url: str, *, headers: Mapping[str, str] | None = None) -> httpx.Response:
        ...
        :raises PsnAuthError: If the response is ``401``/``403`` (an expired/invalid token or authorization).
        """
        return await self._request(PUT_METHOD, url, headers=headers)
    ...
    async def delete(self, url: str, *, headers: Mapping[str, str] | None = None) -> httpx.Response:
        ...
        :raises PsnAuthError: If the response is ``401``/``403`` (an expired/invalid token or authorization).
        """
        return await self._request(DELETE_METHOD, url, headers=headers)
    ...
    async def _send_once(self, method: str, url: str, **kwargs: Any) -> httpx.Response:
        ...
        headers[AUTHORIZATION_HEADER] = f"{BEARER_SCHEME} {token_response[ACCESS_TOKEN_KEY]}"
