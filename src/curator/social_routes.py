...
async def add_friend(online_id: str, request: Request, claims: TokenClaims = Depends(require_bearer)) -> Response:
    ...
    :raises fastapi.HTTPException: 404, if the caller has no PSN link; 403, if ``allow_friend_writes`` is
        not enabled or the mutation cap is spent; 401, if PSN rejects the stored token.
    """
...
async def create_chat_group(
    ...
    :raises fastapi.HTTPException: 404, if the caller has no PSN link; 403, if ``allow_chat_writes`` is
        not enabled or the mutation cap is spent; 401, if PSN rejects the stored token.
    """
    await require_preference(request, claims.sub, CHAT_WRITES)
    service = await _service(request, claims.sub)

    group_id = await _run(service.create_group(online_ids=body.online_ids, account_ids=body.account_ids))
